...
## Deterministic rejection vs ambiguous state (2026-08-08)
Rule: a reconciliation latch is ONLY for genuinely unknowable order states (timeout, no id, non-terminal). A deterministic validation rejection — our own pre-network preflight throws (KrakenPrecisionError etc.) or the venue's explicit refusal to CREATE the order (EOrder minimums, InvalidQuantity, Coinbase success=false) — means NOT ACCEPTED / NO ORDER CREATED and must never latch.
**Why:** the BONK incident — a client-side "Volume below Kraken minimum" throw (raised before any HTTP request) was caught by a blanket `catch` and latched as "unknown order state", pausing the engine over an order that never existed.
**How to apply:** every order path needs (1) live-metadata minimum preflight (Kraken ordermin+costmin, Coinbase base_min_size+min_market_funds, Gemini min_order_size) validated on the FINAL quantized size at fresh prices, (2) an isDeterministicRejection classifier in the submit catch, (3) auto-clear only for latch texts provably client-side (narrow exact-shape regex or typed error). Below-minimum routes: bump to the minimum only if caps (per-action AND rolling daily) + affordability + net-positive all hold; else skip with a cooldown memo so the engine doesn't retry every tick.
